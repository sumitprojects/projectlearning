...
class Home extends CI_Controller {
    ...
    public function home() {
        ...
        $page_data['latest_news'] = simplexml_load_file('https://community.hospitalityconnaisseur.com/category/news/feed/');
    ...
    public function magazines($page = 0) {
        ...
        $page_data['page_title'] = site_phrase('magazines');
    ...
    public function question_papers() {
        ...
        $page_data['page_title'] = site_phrase('question_papers');
        ...
        if (isset($page_data['lesson_id']) && $page_data['lesson_id'] > 0 && ($course_details['course_type'] == 'general' || $course_details['course_type'] == 'magazine' || $course_details['course_type'] == 'question_paper')) {
            ...
            if ($this->session->userdata('role_id') != 1 && $course_details['user_id'] != $this->session->userdata('user_id')) {
                ...
                if (!is_purchased($course_id)) {
                    ...
                    redirect(site_url('courses/'.slugify($course_details['title']).'/'.$course_details['id']), 'refresh');
    ...
    public function subscribe() {
        ...
        $page_data['page_title'] = site_phrase('subscribe');
    ...
    public function contact_us() {
        ...
        $this->form_validation->set_rules('firt_name', 'First Name', 'trim|required|min_length[5]|max_length[50]');
        ...
        if ($this->form_validation->run()) {
            ...
            $data['firt_name'] = $this->input->post('firt_name');
            ...
            $this->email_model->contact_form_notify($data);

            $this->session->set_flashdata('flash_message', site_phrase('thank_you_for_submitting_your_info'));
    ...
    public function advertise_with_us() {
        ...
        $page_data['page_name'] = 'advertise_with_us';

        $page_data['page_title'] = site_phrase('advertise_with_us');
        ...
        $this->form_validation->set_rules('company_name', 'company_name', 'trim|required|min_length[5]|max_length[50]');
        ...
        $page_data['reputed_advertisers'] = $this->db->get('reputed_advertisers')->result_array();
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    ...
    public function jobs($param1 = "", $param2 = ""){
        ...
        if($param1 != '' && $param2 > 0){
            ...
            $page_data['page_name'] = 'career-single';

            $page_data['job'] = $this->db->get_where('jobs',['id'=>$param2,'status'=>1])->row_array();

            $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);

        }else if($param1 == 'apply'){
            ...
            $this->form_validation->set_rules('expected_sal', 'Expected Salary', 'trim|required');
            ...
            $this->form_validation->set_rules('how_know', 'How did you get to know the company?', 'trim|required|min_length[5]|max_length[255]');
            $this->form_validation->set_rules('language[]', 'language', 'required');
            
            $email = $this->input->post('email');
            $phone = $this->input->post('phone');
            $jobid = $this->input->post('job_id');
            $previous_data = $this->db->get_where('job_application', ['email'=>$email, 'phone'=>$phone, 'job_id'=> $jobid])->num_rows();
            $job = $this->db->get_where('jobs',['id'=>$jobid,'status'=>1])->row_array();
            if ($this->form_validation->run() && $previous_data == 0) {
                $application['job_id'] = $jobid;
                $application['name'] = $this->input->post('name');
                $application['email'] = $this->input->post('email');
                $application['phone'] = $this->input->post('phone');
                $application['expected_sal'] = $this->input->post('expected_sal');
                $application['address'] = $this->input->post('address');
                $application['gender'] = $this->input->post('gender');
                $application['how_know'] = $this->input->post('how_know');
                $application['language'] = json_encode($this->input->post('language'));
                ...
                if (isset($_FILES['resume']) && $_FILES['resume']['name'] != "") {
                    ...
                    $accepted_ext = array('doc', 'docs', 'pdf');
                    ...
                    if (in_array(strtolower($ext), $accepted_ext)) {
                        ...
                        move_uploaded_file($_FILES['resume']['tmp_name'], 'uploads/document/'.$document_custom_name);
                        $application['resume'] = 'uploads/document/'.$document_custom_name;
                        $this->db->insert('job_application',$application);
                        $this->session->set_flashdata('success_message', get_phrase('application_accepted'));    
                    }else{    
                        $this->session->set_flashdata('error_message', get_phrase('invalide_file'));
                    }
                }    
            }else{
                $this->session->set_flashdata('error_message', get_phrase('invalide_credential_or_duplicate_entry'));                
            }
            ...
            $page_data['jobs'] = $this->db->get()->result_array();
    ...
    public function newsletter(){
        ...
        $nw['email'] = html_escape($this->input->post('nw_email'));

        $nw['ip_address'] = $_SERVER['REMOTE_ADDR'];

        $nw['email'] = filter_var($nw['email'], FILTER_SANITIZE_EMAIL);



        // Validate e-mail

        if (filter_var($nw['email'], FILTER_VALIDATE_EMAIL)) {

            $newletter = $this->db->insert('newsletter',$nw);

            if($newletter > 0){

                $this->session->set_flashdata('info_message', get_phrase('you_have_already_registered').'. '.get_phrase('thank_you'));

            }else{

                $this->session->set_flashdata('info_message', get_phrase('please_verify_your_email_address'));

...
class Admin extends CI_Controller {
    public function __construct()
    {
        ...
        // init permissions
        // $methods = array_diff(get_class_methods($this),["index","__construct","get_instance","blank_template","backup_database","message","about","install_theme","addon","available_addons"]);
        // foreach ($methods as $method) {
        //     $perm = $this->db->get_where('permission',['name'=>$method])->row_array();
        //     if(empty($perm)){
        //        $this->db->insert('permission',['name'=> $method, 'definition' => $method]);
        //     }
        // }


        $method = $this->uri->segment(2);
        if($this->session->has_userdata('permission') && $this->session->userdata('role_id') > 2){
            if(!in_array($method, $this->session->userdata('permission'))){
    ...
    public function get_courses() {
        ...
        if(!empty($courses)) {
            foreach ($courses as $key => $row) {
                ...
                $view_course_on_frontend_url = site_url('courses/'.rawurlencode(slugify($row->title)).'/'.$row->id);
    ...
    public function magazine_actions($param1 = "", $param2 = "") {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($param1 == "add") {
            ...
            redirect(site_url('admin/magazines'), 'refresh');
        }
        elseif ($param1 == 'delete') {
            $this->is_drafted_course($param2);
            $this->crud_model->delete_course($param2);
    ...
    public function magazine_form($param1 = "", $param2 = "") {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }

        if ($param1 == 'add_magazine') {
            $page_data['languages'] = $this->crud_model->get_all_languages();
            $page_data['categories'] = $this->crud_model->get_categories('','magazines');
            $page_data['page_name'] = 'magazine_add';
            $page_data['page_title'] = get_phrase('add_magazine');
            $this->load->view('backend/index', $page_data);

        }elseif ($param1 == 'magazine_edit') {
            $this->is_drafted_course($param2);
            $page_data['page_name'] = 'magazine_edit';
            $page_data['course_id'] =  $param2;
            $page_data['page_title'] = get_phrase('edit_magazine');
            $page_data['languages'] = $this->crud_model->get_all_languages();
            $page_data['categories'] = $this->crud_model->get_categories('','magazines');
            $this->load->view('backend/index', $page_data);
        }
    ...
    public function magazines() {
        ...
        // Courses query is used for deciding if there is any course or not. Check the view you will get it
        $page_data['courses']                = $this->crud_model->filter_course_for_backend($page_data['selected_category_id'], $page_data['selected_instructor_id'], $page_data['selected_price'], $page_data['selected_status'],'magazine');
        $page_data['status_wise_courses']    = $this->crud_model->get_status_wise_courses("",'magazine');
        $page_data['instructors']            = $this->user_model->get_instructor()->result_array();
        $page_data['page_name']              = 'magazines-server-side';
        $page_data['categories']             = $this->crud_model->get_categories('','magazines');
        $page_data['page_title']             = get_phrase('active_courses');
        $this->load->view('backend/index', $page_data);
    }
    ...
    public function get_magazines() {
        ...
        $totalData = $this->lazyload->count_all_courses($filter_data,'magazine');
        $totalFiltered = $totalData;

        // This block of code is handling the search event of datatable
        if(empty($this->input->post('search')['value'])) {
            $courses = $this->lazyload->courses($limit, $start, $order, $dir, $filter_data,'magazine');
        }
        else {
            $search = $this->input->post('search')['value'];
            $courses =  $this->lazyload->course_search($limit, $start, $search, $order, $dir, $filter_data,'magazine');
            $totalFiltered = $this->lazyload->course_search_count($search);
        }
        ...
        if(!empty($courses)) {
            foreach ($courses as $key => $row) {
                ...
                $view_course_on_frontend_url = site_url('magazines/'.rawurlencode(slugify($row->title)).'/'.$row->id);
                $edit_this_course_url = site_url('admin/magazine_form/magazine_edit/'.$row->id);
                $section_and_lesson_url = site_url('admin/magazine_form/magazine_edit/'.$row->id);
                ...
                $delete_course_url = "confirm_modal('".site_url('admin/magazine_actions/delete/'.$row->id)."')";
                ...
                <ul class="dropdown-menu">
                <li><a class="dropdown-item" href="'.$view_course_on_frontend_url.'" target="_blank">'.get_phrase("view_magazine_on_frontend").'</a></li>
                <li><a class="dropdown-item" href="'.$edit_this_course_url.'">'.get_phrase("edit_this_magazine").'</a></li>
                '.$section_and_lesson_menu.'
                <li><a class="dropdown-item" href="javascript::" onclick="'.$course_status_changing_action.'">'.$course_status_changing_message.'</a></li>
                <li><a class="dropdown-item" href="javascript::" onclick="'.$delete_course_url.'">'.get_phrase("delete").'</a></li>
                </ul>
                ...
                $nestedData['title'] = '<strong><a href="'.site_url('admin/magazine_form/magazine_edit/'.$row->id).'">'.$row->title.'</a></strong><br>
                <small class="text-muted">'.get_phrase('instructor').': <b>'.$instructor_details['first_name'].' '.$instructor_details['last_name'].'</b></small>';
                ...
                if($row->course_type == 'scorm'){
                    $nestedData['lesson_and_section'] = '<span class="badge badge-info-lighten">'.get_phrase('scorm_course').'</span>';
                }elseif($row->course_type == 'magazine'){
                    $nestedData['lesson_and_section'] = '
                    <small class="text-muted"><b>'.get_phrase('total_section').'</b>: '.$sections->num_rows().'</small><br>
                    <small class="text-muted"><b>'.get_phrase('total_lesson').'</b>: '.$lessons->num_rows().'</small>';
                }

                $nestedData['enrolled_student'] = '<small class="text-muted"><b>'.get_phrase('total_purchase').'</b>: '.$enroll_history->num_rows().'</small>';
    ...
    public function question_paper_actions($param1 = "", $param2 = "") {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($param1 == "add") {
            $course_id = $this->crud_model->add_course();
            redirect(site_url('admin/question_paper_form/question_paper_edit/'.$course_id), 'refresh');

        }elseif ($param1 == "edit") {
            $this->crud_model->update_course($param2);

            redirect(site_url('admin/question_papers'), 'refresh');
        }
        elseif ($param1 == 'delete') {
            $this->is_drafted_course($param2);
            $this->crud_model->delete_course($param2);
    ...
    public function question_paper_form($param1 = "", $param2 = "") {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }

        if ($param1 == 'add_question_paper') {
            $page_data['languages'] = $this->crud_model->get_all_languages();
            $page_data['categories'] = $this->crud_model->get_categories('','question-papers');
            $page_data['page_name'] = 'question_paper_add';
            $page_data['page_title'] = get_phrase('add_question_paper');
            $this->load->view('backend/index', $page_data);

        }elseif ($param1 == 'question_paper_edit') {
            $this->is_drafted_course($param2);
            $page_data['page_name'] = 'question_paper_edit';
            $page_data['course_id'] =  $param2;
            $page_data['page_title'] = get_phrase('edit_question_paper');
            $page_data['languages'] = $this->crud_model->get_all_languages();
            $page_data['categories'] = $this->crud_model->get_categories('','question-papers');
            $this->load->view('backend/index', $page_data);
        }
    ...
    public function question_papers() {
        ...
        // Courses query is used for deciding if there is any course or not. Check the view you will get it
        $page_data['courses']                = $this->crud_model->filter_course_for_backend($page_data['selected_category_id'], 'all', $page_data['selected_price'], $page_data['selected_status'],'question_paper');
        $page_data['page_name']              = 'question_paper-server-side';
        $page_data['categories']             = $this->crud_model->get_categories('','question_papers');
        $page_data['page_title']             = get_phrase('active_courses');        $this->load->view('backend/index', $page_data);
    }

    // This function is responsible for loading the course data from server side for datatable SILENTLY
    public function get_question_papers() {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        $courses = array();
        ...
        $filter_data['selected_instructor_id']     = 'all';
        // Server side processing portion
        $columns = array(
            0 => '#',
            1 => 'title',
        ...
        $totalData = $this->lazyload->count_all_courses($filter_data,'question_paper');
        $totalFiltered = $totalData;
        // This block of code is handling the search event of datatable
        if(empty($this->input->post('search')['value'])) {
            $courses = $this->lazyload->courses($limit, $start, $order, $dir, $filter_data,'question_paper');
        }
        else {
            $search = $this->input->post('search')['value'];
            $courses =  $this->lazyload->course_search($limit, $start, $search, $order, $dir, $filter_data,'question_paper');
            $totalFiltered = $this->lazyload->course_search_count($search);
        }
        ...
        if(!empty($courses)) {
            foreach ($courses as $key => $row) {
                ...
                $view_course_on_frontend_url = site_url('question-papers/'.rawurlencode(slugify($row->title)).'/'.$row->id);
                $edit_this_course_url = site_url('admin/question_paper_form/question_paper_edit/'.$row->id);
                $section_and_lesson_url = site_url('admin/question_paper_form/question_paper_edit/'.$row->id);
                ...
                $delete_course_url = "confirm_modal('".site_url('admin/question_paper_actions/delete/'.$row->id)."')";
                ...
                <ul class="dropdown-menu">
                <li><a class="dropdown-item" href="'.$view_course_on_frontend_url.'" target="_blank">'.get_phrase("view_question_paper_on_frontend").'</a></li>
                <li><a class="dropdown-item" href="'.$edit_this_course_url.'">'.get_phrase("edit_this_question_paper").'</a></li>
                '.$section_and_lesson_menu.'
                <li><a class="dropdown-item" href="javascript::" onclick="'.$course_status_changing_action.'">'.$course_status_changing_message.'</a></li>
                <li><a class="dropdown-item" href="javascript::" onclick="'.$delete_course_url.'">'.get_phrase("delete").'</a></li>
                </ul>
                ...
                $nestedData['title'] = '<strong><a href="'.site_url('admin/question_paper_form/question_paper_edit/'.$row->id).'">'.$row->title.'</a></strong><br>
                <small class="text-muted">'.get_phrase('instructor').': <b>'.$instructor_details['first_name'].' '.$instructor_details['last_name'].'</b></small>';
                ...
                if($row->course_type == 'scorm'){
                    $nestedData['lesson_and_section'] = '<span class="badge badge-info-lighten">'.get_phrase('scorm_course').'</span>';
                }elseif($row->course_type == 'question_paper'){
                    $nestedData['lesson_and_section'] = '
                    <small class="text-muted"><b>'.get_phrase('total_section').'</b>: '.$sections->num_rows().'</small><br>
                    <small class="text-muted"><b>'.get_phrase('total_lesson').'</b>: '.$lessons->num_rows().'</small>';
                }
    ...
    public function sections($param1 = "", $param2 = "", $param3 = "") {
        ...
        $course = $this->db->get_where('course',['id'=>$param1])->row_array();        if($course['type'] == 'general'){            redirect(site_url('admin/course_form/course_edit/'.$param1));        }else{

            redirect(site_url('admin/'.$course['course_type'].'_form/'.$course['course_type'].'_edit/'.$param1));
    ...
    public function lessons($course_id = "", $param1 = "", $param2 = "") {
        ...
        if ($param1 == 'add') {
            ...
            if($course['course_type'] != 'general'){                redirect(site_url('admin/'.$course['course_type'].'_form/'.$course['course_type'].'_edit/'.$course_id));            }else{
        ...
        elseif ($param1 == 'edit') {
            ...
            if($course['course_type'] != 'general'){
                redirect(site_url('admin/'.$course['course_type'].'_form/'.$course['course_type'].'_edit/'.$course_id));
            }else{
                redirect(site_url('admin/course_form/course_edit/'.$course_id));
            }
        }
    ...
    }    public function manage_newsletter($param1 = "", $param2 = ""){
        if ($this->session->userdata('admin_login') != 1)
        redirect(site_url('login'), 'refresh');

        if ($param1 == 'approve' || $param1 == 'delete') {
            $this->db->update('newsletter',['status'=>($param1=='approve')?1:0],['id'=>$param2]);
        }
        $page_data['page_name']  = 'newsletter_list';
        $page_data['page_title'] = get_phrase('newsletters');
        $page_data['newsletters'] = $this->db->get('newsletter')->result_array();
        $this->load->view('backend/index', $page_data);
    }

    public function manage_advertiser($param1 = "", $param2 = ""){
        if ($this->session->userdata('admin_login') != 1)
        redirect(site_url('login'), 'refresh');

        if ($param1 == 'approve' || $param1 == 'delete') {
            $this->db->update('advertiser',['status'=>($param1=='approve')?1:0],['id'=>$param2]);
        }
        $page_data['page_name']  = 'advertiser_list';
        $page_data['page_title'] = get_phrase('advertiser');
        $page_data['advertisers'] = $this->db->get('advertiser')->result_array();
        $this->load->view('backend/index', $page_data);
    }    // reputed advertiser list

    public function reputed_advertisers($param1 = "", $param2 = ""){

        if ($this->session->userdata('admin_login') != 1)
        redirect(site_url('login'), 'refresh');
        if($param1 == 'add'){
            $advertiser['title']  = html_escape($this->input->post('title')); 
            $advertiser['url']  = html_escape($this->input->post('url')); 

            $advertiser['slug'] = slugify($advertiser['title']);
            $advertiserid  = html_escape($this->input->post('id')); 
            if (isset($_FILES['image']) && $_FILES['image']['name'] != "") {

                if($advertiserid > 0){

                    $adv = $this->db->get_where('reputed_advertisers',['id'=>$param2])->row_array();

                    if(isset($adv['image']))unlink('upload/system'.$adv['image']);

                }

                $advertiser['image'] = md5(rand(1000, 100000)).'.png';

    

                move_uploaded_file($_FILES['image']['tmp_name'], 'uploads/system/'.$advertiser['image']);

            }
            if($advertiserid> 0){

                $this->db->update('reputed_advertisers',$advertiser,['id'=>$advertiserid]);

            }else{

                $this->db->insert('reputed_advertisers',$advertiser);

            }

            

            redirect('admin/reputed_advertisers','refresh');
        ...
        if ($param1 == 'delete') {
            ...
            $this->db->delete('reputed_advertiser',['id'=>$param2]);
        ...
        if($param1 == 'edit' && $param2>0){
            ...
            $page_data['advertiser'] = $this->db->get_where('reputed_advertisers',['id'=>$param2])->row_array();

        }

        

        $page_data['page_name']  = 'reputed_advertiser_list';
        $page_data['page_title'] = get_phrase('reputed_advertisers');
        $page_data['advertisers'] = $this->db->get('reputed_advertisers')->result_array();
        $this->load->view('backend/index', $page_data);

    }
    // manage_job
    public function manage_jobs($param1 = "", $param2 = ""){
        if ($this->session->userdata('admin_login') != 1)
        redirect(site_url('login'), 'refresh');

        if ($param1 == 'approve' || $param1 == 'delete') {
            $this->db->update('jobs',['status'=>($param1=='approve')?1:0],['id'=>$param2]);
        ...
        if($param1 == 'edit' && $param2>0){
            ...
            $this->form_validation->set_rules('location', 'location', 'trim|required|min_length[5]|max_length[100]');
            ...
            if ($this->form_validation->run()) {
                ...
                if($id > 0){
                    $this->db->update('jobs',$job,['id'=>$id]);
                    $this->session->set_flashdata('flash_message', get_phrase('job_added_successfully'));    
        ...
        $page_data['page_name']  = 'job_list';
        $page_data['page_title'] = get_phrase('jobs');
        $page_data['jobs'] = $this->db->get('jobs')->result_array();
        $this->load->view('backend/index', $page_data);
    }    public function manage_applicants($param1 = '', $param2 = ''){
        if($param1 == 'job' && $param2 > 0){
            $page_data['pending_applications'] = $this->db->get_where('job_application',['resume_status'=>'pending']);

            $page_data['approved_applications'] = $this->db->get_where('job_application',['resume_status'=>'approve']);

            $page_data['rejected_applications'] = $this->db->get_where('job_application',['resume_status'=>'reject']);

        }

        else if($param1 == 'approve' && $param2 > 0){

            $application = $this->db->get_where('job_application',['id'=>$param2])->row_array();

            $this->db->update('job_application',['resume_status'=>'approve'],['id'=>$param2,'resume_status'=>'pending']);

            redirect('admin/manage_applicants/job/'.$application['job_id'],'refresh');

        }

        else if($param1 == 'reject' && $param2 > 0){

            $application = $this->db->get_where('job_application',['id'=>$param2])->row_array();

            $this->db->update('job_application',['resume_status'=>'reject'],['id'=>$param2,'resume_status'=>'pending']);

            redirect('admin/manage_applicants/job/'.$application['job_id'],'refresh');

        }
        $page_data['page_name']  = 'applicant_list';
        $page_data['page_title'] = get_phrase('jobs_applicant');
        $page_data['jobs'] = $this->db->get('jobs')->result_array();
        $this->load->view('backend/index', $page_data);
    }    public function manage_faq($param1 = "", $param2 = ""){
        if ($this->session->userdata('admin_login') != 1)
        redirect(site_url('login'), 'refresh');

        if ($param1 == 'delete') {
            $this->db->update('faqs',['status'=>0],['id'=>$param2]);

            redirect(site_url('admin/manage_faq'), 'refresh');

        }

        if($param1 == 'edit' && $param2>0){

            $page_data['faq'] = $this->db->get_where('faqs',['id'=>$param2])->row_array();

        }

        if($param1 == 'add'){
            $this->form_validation->set_rules('question', 'question', 'trim|required|min_length[5]|max_length[100]');

            $this->form_validation->set_rules('answer', 'answer', 'trim|required');

            if ($this->form_validation->run()) {
                $faqs['question'] = $this->input->post('question');
                $faqs['answer'] = $this->input->post('answer');
                $id = $this->input->post('id');
                if($id > 0){
                    $this->db->update('faqs',$faqs,['id'=>$id]);
                    $this->session->set_flashdata('flash_message', get_phrase('faq_added_successfully'));    
    ...
    public function manage_dictionary($param1 = '', $param2 = '', $param3 = '',$param4 = ''){
        ...
        if ($param1 == 'add_language') {
            $language = ucwords($this->input->post('language'));            if ($language == 'n-a') {
                $this->session->set_flashdata('error_message', get_phrase('language_name_can_not_be_empty_or_can_not_have_special_characters'));
        ...
        if ($param1 == 'add_phrase') {
            $lang['phrase'] = htmlspecialchars($this->input->post('phrase'));
            $lang['slug'] = slugify($lang['phrase']);
            $lang['meaning'] = htmlspecialchars($this->input->post('meaning'));
            $lang['pronounce'] = htmlspecialchars($this->input->post('pronounce'));
            $lang['lang_key'] = $this->input->post('lang_key');
            $this->db->insert('dictionary',$lang);
            echo $lang['slug'].' '.get_phrase('has_been_added_successfully');
    ...
    public function get_dictionary(){
        ...
        if(!empty($phrase_list)){
            ...
            foreach($phrase_list as $key => $value){
                ...
                $nestedData['updated_phrase'] = '<input type="text" class="form-control" name="updated_phrase" value="'.$value['phrase'].'" id="phrase-'.$value['id'].'">';

                    

                $nestedData['updated_meaning'] = '<input type="text" class="form-control" name="updated_meaning" value="'.$value['meaning'].'" id="meaning-'.$value['id'].'">';
                $nestedData['updated_pronounce'] = '<input type="text" class="form-control" name="updated_pronounce" value="'.$value['pronounce'].'" id="pronounce-'.$value['id'].'">';
                $nestedData['action'] = '<button type="button" class="btn btn-icon btn-primary" id="btn-'.$key.'" onclick="updatePhrase('.trim($value['id']).','.trim($value['lang_key']).')"> <i class="mdi mdi-check-circle"></i> </button>';
        ...
        $json_data = array(
        "draw" => intval($this->input->post('draw')),
        "recordsTotal" => intval(count($phrase_list)),
        "recordsFiltered" => intval(count($phrase_list)),
        "data" => $data
        );

        echo json_encode($json_data);

    }    public function update_dic_phrase_with_ajax() {
        $current_editing_language = $this->input->post('currentEditingLanguage');
        $lang['phrase'] = htmlspecialchars($this->input->post('phrase'));
        $lang['slug'] = slugify($lang['phrase']);
        $lang['meaning'] = htmlspecialchars($this->input->post('meaning'));
        ...
        $key = $this->input->post('key');
        $this->db->update('dictionary',$lang,['id'=>$key,'lang_key'=>$lang_key]);
        echo $current_editing_language.' '.$lang['slug'].' '.$updatedValue;

...

{
    ...
    public function get_categories($param1 = "",$slug='')
    {
        ...
        if($slug == 'general'){
            $this->db->where('slug <>', 'magazines',true);
            $this->db->where('slug <>', 'question-papers',true);
        }
        else if($slug != ''){
...

    {
        ...
        $data['value'] = html_escape($this->input->post('support_time'));

        $this->db->where('key', 'support_time');

        $this->db->update('settings', $data);



        $data['value'] = html_escape($this->input->post('facebook_link'));

        $this->db->where('key', 'facebook_link');

        $this->db->update('settings', $data);



        $data['value'] = html_escape($this->input->post('twitter_link'));

        $this->db->where('key', 'twitter_link');

        $this->db->update('settings', $data);



        $data['value'] = html_escape($this->input->post('linkedin_link'));

        $this->db->where('key', 'linkedin_link');

        $this->db->update('settings', $data);



        $data['value'] = html_escape($this->input->post('youtube_link'));
        ...
        $data['value'] = html_escape($this->input->post('instagram_link'));
        ...
        $data['value'] = html_escape($this->input->post('whatsapp_link'));
        ...
        $data['value'] = html_escape($this->input->post('notification_bar'));

        $this->db->where('key', 'notification_bar');

        $this->db->update('settings', $data);



        $data['value'] = html_escape($this->input->post('notification_link'));

        $this->db->where('key', 'notification_link');
...

    {
        ...
        if($course_type == 'general' || $course_type == 'magazine'){
    ...
    public function get_status_wise_courses($status = "",$type = 'general')

    {
        ...
        if(addon_status('scorm_course')){
            ...
            if ($status != "" ) {
                ...
                $courses = $this->db->get_where('course', array('status' => $status, 'course_type' => $type));

            } else {

                $courses['draft'] = $this->db->get_where('course', array('status' => 'draft', 'course_type' => $type));

                $courses['pending'] = $this->db->get_where('course', array('status' => 'pending', 'course_type' => $type));

                $courses['active'] = $this->db->get_where('course', array('status' => 'active', 'course_type' => $type));
...

    {
        ...
        $data['value'] = html_escape($this->input->post('heading_line'));

        $this->db->where('key', 'heading_line');
        ...
        $data['value'] = $_POST['tawk_plugin'];

        $this->db->where('key', 'tawk_plugin');

        $this->db->update('frontend_settings', $data);



        $data['value'] = $_POST['google_site_verification'];

        $this->db->where('key', 'google_site_verification');

        $this->db->update('frontend_settings', $data);



        $data['value'] = $_POST['google_analytics_code'];

        $this->db->where('key', 'google_analytics_code');

        $this->db->update('frontend_settings', $data);



        $data['value'] = $_POST['google_adsence_plugin'];

        $this->db->where('key', 'google_adsence_plugin');
        ...
        $popup_settings = [
            ...
            'popup_timer'  =>   $this->input->post('popup_timer')
        ];

        if (isset($_FILES['image_link']) && $_FILES['image_link']['name'] != "") {

            unlink('uploads/system/' . json_decode(get_frontend_settings('popup_settings'))->image_link);

            $popup_settings['image_link'] = md5(rand(1000, 100000)).'.png';

            move_uploaded_file($_FILES['image_link']['tmp_name'], 'uploads/system/'.$popup_settings['image_link']);

        }else{
            $popup_settings['image_link'] = "";
        }

        $this->db->where('key', 'popup_settings');

        $this->db->update('frontend_settings', ['value'=>json_encode($popup_settings)]);
...

    {
        ...
        $this->db->select("co.*,ct.name as 'category',ct.slug as 'c_slug'");

        $this->db->from('course as co');

        $this->db->join('category as ct','co.sub_category_id = ct.id');

        $this->db->order_by("ct.id", "desc");

        $this->db->where('co.status', 'active');
...

    {
        ...
        if ($course_details['is_free_course'] == 1) {
            ...
            redirect(site_url('courses/' . slugify($course_details['title']) . '/' . $course_id), 'refresh');
    ...
    public function filter_course_for_backend($category_id, $instructor_id, $price, $status, $type = 'general')
    ...
    public function get_free_and_paid_courses($price_status = "", $instructor_id = "", $type='general')
